import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import type { ContentItem } from "@/features/content/types/content";

type LibraryDetailPanelProps = {
  item: ContentItem;
};

export function LibraryDetailPanel({ item }: LibraryDetailPanelProps) {
  return (
    <Card className="border-white/10 bg-zinc-900/60">
      <CardHeader>
        <Badge className="w-fit bg-amber-400/15 text-amber-300 hover:bg-amber-400/15">
          Recurso de lectura
        </Badge>
        <CardTitle className="text-white">Detalle del material</CardTitle>
      </CardHeader>

      <CardContent className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl bg-zinc-950/70 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-zinc-500">Formato</p>
            <p className="mt-2 text-sm font-medium text-zinc-200">
              {item.format ?? "Documento digital"}
            </p>
          </div>

          <div className="rounded-2xl bg-zinc-950/70 p-4">
            <p className="text-xs uppercase tracking-[0.2em] text-zinc-500">Extensión</p>
            <p className="mt-2 text-sm font-medium text-zinc-200">
              {item.pages ? `${item.pages} páginas` : "No especificado"}
            </p>
          </div>
        </div>

        <div className="rounded-2xl border border-white/10 bg-zinc-950/60 p-5">
          <p className="text-sm leading-7 text-zinc-400">
            Este material está orientado a lectura, consulta y acceso educativo
            on-demand, con una experiencia pensada para recursos digitales.
          </p>
        </div>

        <div className="flex flex-wrap gap-3">
          <Button className="bg-amber-400 text-zinc-950 hover:bg-amber-300">
            Leer recurso
          </Button>
          <Button variant="outline">Descargar ficha</Button>
        </div>
      </CardContent>
    </Card>
  );
}